...
class MangaFormPage extends StatefulWidget {
  ...
  final MangaEntity? initialManga;
  final List<AuthorEntity> authors;
  final List<GenreEntity> genres;
  final String Function(String?) normalizeStatus;
  final MangaFormSubmit onSubmit;
  ...
  @override
  State<MangaFormPage> createState() => _MangaFormPageState();
}
...
class _MangaFormPageState extends State<MangaFormPage> {
  ...
  MangaEntity? get _initialManga => widget.initialManga;
  bool get _isEditing => _initialManga != null;

  @override
  void initState() {
    super.initState();

    final MangaEntity? manga = _initialManga;
    _titleController = TextEditingController(text: manga?.title ?? '');
    _descriptionController = TextEditingController(
      text: manga?.description ?? '',
    );
    _thumbnailController = TextEditingController(text: manga?.thumbnail ?? '');
    _totalChapterController = TextEditingController(
      text: '${manga?.totalChapter ?? 0}',
    );
    _rateController = TextEditingController(text: '${manga?.rate ?? 0}');

    _releaseDate = _normalizeDate(manga?.releaseDate);
    _endDate = _normalizeDate(manga?.endDate);
    final String normalizedStatus = widget.normalizeStatus(manga?.status);
    _status = _statusOptions.contains(normalizedStatus)
        ? normalizedStatus
        : _statusOptions.first;
    _authorId = _resolveAuthorId();
    _selectedGenreIds =
        manga?.genreIds?.where((id) => id > 0).toSet() ?? <int>{};
  }
  ...
  int? _resolveAuthorId() {
    final int? currentAuthorId = _initialManga?.authorId;
  ...
  void _submit() {
    ...
    final MangaEntity manga = MangaEntity(
      id: _initialManga?.id,
  ...
  Widget _buildThumbnailPicker() {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const _SectionTitle('Thumbnail', small: true),
        const SizedBox(height: 8),
        Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
  ...
  Widget _buildGenrePicker(List<GenreEntity> validGenres) {
    ...
    return Wrap(
      ...
      children: validGenres.map((genre) {
        ...
        final String label = (genre.name ?? '').trim().isEmpty
            ? 'Thể loại #$genreId'
            : genre.name!.trim();
